...
def read07Excel(path):
    ...
    for i in range(0,len(sheets)) :
        ...
        for row in sheet.rows:
            if(row[1].value == '没有'):
                if(row[5].value != '无'):
                    search_isbn(out,url,row[5].value,row)
                else :
                    i = i + 1
                    print('i =',i)
    out.close()
...
def read07_excel(path): #处理高等植物彩色图鉴的数据
    wb = openpyxl.load_workbook(path)
    sheets = wb.sheetnames
    ...
    for index,row in enumerate(sheet.rows):
        ...
        if row[2].value is not None: #种
            ...
            if len(mings) > 1:
                ...
                latin1 = arr[0]
                shu = mings[0]
                zuozhe = ';'.join(arr[1:len(arr)])
                if shu is not None:
                    #if shu not in is_existed_kes:
                    shu_data = get_name_existed(shu,mark)
                    if shu_data is None:
                        shu_data_existed = select('select * from tb_classsys where del_flag=0 and classsys_latin2=\'%s\'' % (shu))
                        if shu_data_existed is not None:
                            #if pid != shu_data_existed[3]:#属的上一级归类不正确
                            ...
                            sql = sql % (18, '属', pid, shu, shu_data_existed[5], shu_data_existed[6], shu, get_pinyin(shu_data_existed[6]),
                                         get_pinyin_prefix(shu_data_existed[6]),
                                         datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                         1, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0,mark)
                            shu_id = insertMysql(sql)
                        else:
                            sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`type1`,`create_date`,`del_flag`,`mark`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                            sql = sql % (18, '属', pid, shu, '', '', shu, '','',datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),1, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0,mark)
                            shu_id = insertMysql(sql)
                    else:
                        if shu_data[3] != pid:
                            update('update tb_classsys set classsys_par=%s where id=%s' % (pid,shu_data[0]))
                        shu_id = shu_data[0]
                zhong_data = select('select * from tb_classsys where mark=%s and del_flag=0 and classsys_cname=\'%s\' and classsys_latin2=\'%s\'' % (mark,row[0].value,latin1))
                if zhong_data is not None:
                    if zhong_data[3] != shu_id:
                        update('update tb_classsys set classsys_par=%s where id=%s' % (shu_id, zhong_data[0]))
                    else:
                        print('相同')
                    #update('update tb_classsys set classsys_author=\'%s\' where id=%s' % (zuozhe, zhong_data[0]))
                else:
                    sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`type1`,`create_date`,`del_flag`,`mark`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                    sql = sql % (26, 'sp.', shu_id, latin1, pymysql.escape_string(zuozhe), row[0].value, latin1, get_pinyin(row[0].value),
                             get_pinyin_prefix(row[0].value), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1,
                             datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0,mark)
                    cid = insertMysql(sql)
            ...
            zhiwu_data = select('select * from tb_classsys where mark=%s and del_flag=0 and classsys_cname=\'%s\' and classsys_latin2=\'%s\'' % (mark,row[0].value,row[1].value))
            if zhiwu_data is None:
                print('科：',row[0].value, row[1].value)
                zhiwu_data_existed = select('select * from tb_classsys where del_flag=0 and classsys_cname=\'%s\' and classsys_latin2=\'%s\'' % (row[0].value, row[1].value))
                if zhiwu_data_existed is not None:
                    sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`type1`,`create_date`,`del_flag`,`mark`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                    sql = sql % (13, '科', 0, zhiwu_data_existed[4], zhiwu_data_existed[5], zhiwu_data_existed[6], zhiwu_data_existed[8], get_pinyin(zhiwu_data_existed[8]),
                                 get_pinyin_prefix(zhiwu_data_existed[8]), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                 1, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0,mark)
                    pid = insertMysql(sql)
                else:
                    sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`type1`,`create_date`,`del_flag`,`mark`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                    sql = sql % (13, '科', 0, row[1].value, '', row[0].value, row[1].value, get_pinyin(row[0].value),
                                 get_pinyin_prefix(row[0].value), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                 1, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0,mark)
                    pid = insertMysql(sql)
            else:
                pid = zhiwu_data[0]
    ...
    for index,row in enumerate(sheet.rows):
        ...
        if row[2].value is not None: #种
            ...
            if len(mings) > 1:
                ...
                shu = mings[0]
                if row[3].value is not None :
                    ke_data_existed = get_name_existed_create_date(row[3].value,False)
                    if ke_data_existed is None:
                        ke_data = get_name_existed_create_date(row[3].value)
                        print('科信息：',ke_data)
                        if ke_data is not None:
                            sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                            ...
                            sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                            sql = sql % (13, '科', pid, '', '', row[3].value, '', get_pinyin(row[3].value),
                            get_pinyin_prefix(row[3].value),
                            datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
                            ke_id = insertMysql(sql)
                    else:
                        ke_id = ke_data_existed[0]
                if shu is not None:
                    shu_data_existed = get_latin2_existed(shu,False)
                    print(shu_data_existed)
                    if shu_data_existed is None:
                        ...
                        if shu_data is not None:
                            ...
                            sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                            sql = sql % (18, '属', ke_id, shu, '', '', shu, '','',
                                         datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                         datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
                            shu_id = insertMysql(sql)
                    else:
                        if shu_data_existed[3] != ke_id :
                            update('update tb_classsys set classsys_par=%s where id=%s' % (ke_id,shu_data_existed[0]))
                        shu_id = shu_data_existed[0]
                wuzhong_data = select('select * from tb_classsys where del_flag=0 and classsys_latin2=\'%s\' and classsys_cname=\'%s\' and create_date > \'2019-04-17\'' % (latin,row[4].value))
                if wuzhong_data is None:
                    sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`create_date`,`del_flag`,`type1`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                    sql = sql % (26, 'sp.', shu_id, latin, '', row[4].value, latin, get_pinyin(row[4].value),
                             get_pinyin_prefix(row[4].value), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                             datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0,row[5].value)
                    cid = insertMysql(sql)
                    print(sql)
                    if (row[6].value is not None):
                        sql1 = "insert into tb_spdesc (`spcid`,`splatin2`,`spdescid`,`spdesc`,`sporderid`,`spaddtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s')"
                        sql1 = sql1 % (
                        cid, latin, 7, row[6].value, 7, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
                        print(sql1)
                        insertMysql(sql1)
                    if (row[7].value is not None):
                        sql1 = "insert into tb_spdesc (`spcid`,`splatin2`,`spdescid`,`spdesc`,`sporderid`,`spaddtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s')"
                        sql1 = sql1 % (
                        cid, latin, 4, row[7].value, 4, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
                        print(sql1)
                        insertMysql(sql1)
                else:
                    if wuzhong_data[3] != shu_id :
                        update('update tb_classsys set classsys_par=%s where id=%s' % (shu_id, wuzhong_data[0]))
...
def read07_excel_gdzwtj(path):
    wb = openpyxl.load_workbook(path)
    sheets = wb.sheetnames
    sheet = sheet = wb.get_sheet_by_name(sheets[0])
    for index_no,row in enumerate(sheet.rows):
        #print(row[0].value,row[1].value,row[2].value,row[3].value,row[4].value,row[5].value,row[6].value)
        gdzwtj_data = select('select * from tb_classsys where del_flag=0 and mark is null and create_date > \'2019-2-14\' and classsys_classid=26 and classsys_latin2=\'%s\'' % row[2].value)
        if gdzwtj_data is None:
            print(row[0].value,gdzwtj_data)
        else:
            ke_ming = row[3].value
            print(row[0].value,ke_ming)
            update('update tb_classsys set classsys_bz=\'%s\' where id = %s' % (ke_ming,gdzwtj_data[0]))

        #if index_no > 10:break
    wb.close()


def get_latin2_existed(name,dangqian=True):
    db = pymysql.connect(host='localhost',port= 3306,user = 'root',passwd='123456',db='zhiwu',charset='utf8')
    cursor = db.cursor()
    sql = 'select * from tb_classsys where classsys_latin2=\'%s\' and del_flag=0'
    if dangqian :
        print(sql)
    else:
        sql = sql + ' and create_date > \'2019-04-17\''
    sql = sql % name
    print(sql)
    cursor.execute(sql)
...
def get_name_existed(name,mark):
    db = pymysql.connect(host='localhost',port= 3306,user = 'root',passwd='123456',db='zhiwu',charset='utf8')
    cursor = db.cursor()
    sql = 'select * from tb_classsys where classsys_latin2=\'%s\' and mark=%s and del_flag=0'
    sql = sql % (name,mark)
    cursor.execute(sql)
    data = cursor.fetchone()
    #print(data)
...
def get_name_existed_create_date(name_cn,dangqian=True):
    db = pymysql.connect(host='localhost',port= 3306,user = 'root',passwd='123456',db='zhiwu',charset='utf8')
    cursor = db.cursor()
    sql = 'select * from tb_classsys where classsys_cname=\'%s\' and del_flag=0'
    if dangqian :
        print(sql)
    else:
        sql = sql + ' and create_date > \'2019-04-17\''
...
def get_text_docx():
    file = docx.Document("C:\\Users\\dell\\Desktop\\高等彩九卷-更新.docx")
    i = 1
    j = 0
    wb = xlwt.Workbook()
    ws = wb.add_sheet('中国高等植物彩色图鉴正文内容-高等彩九卷-更新', cell_overwrite_ok=True)
    ws.write(0, 0, '物种中文名')
    ...
    wb.save("C:/Users/dell/Desktop/中国高等植物高等彩九卷-更新.xls")
...
def get_parents_classsys(book_id,pid):
    datas = select(
        'select * from tb_classsys where classsys_par = %s and id<=72182 and del_flag = 0' % pid,
        'zhiwu', False)
...
def zhiwu_book_relates():
    ...
    book_datas = select('select * from tb_book where id=163', 'zhiwu', False)
    # print(book_datas)
    for book in book_datas:
        ...
        for ke in ke_arr:
            data = select(
                'select * from tb_classsys where classsys_cname = \'%s\' and id<=72182 and del_flag = 0' % ke,
                'zhiwu')
            if data is not None:
                print(data)
                data_id = data[0]
...
def deal_with_gdzwcstj():
    ...
    for data in foc_datas:
        ...
        cname = data[4]
        zhong = select(
            'select * from tb_classsys where del_flag=0 and mark in (202,196,188,189,190,191,192,193,194) and classsys_latin2 = \'%s\'' % pymysql.escape_string(
                latin2.strip()), 'zhiwu', True)
...
def deal_with_foc():
    ...
    data_count = select('select count(*) from zhiwu2 where id not in (select zhiwuid from tb_classsys where del_flag = 5 and classsys_classid=26) order by id', 'zhiwu', True)
    ...
    for index in range(0,page_index+1):
        if index >= 17:break
        ...
        sql = 'select id,juan,xueming,zhwenming,ke,shu,zaipei,yisheng,hei,ji,liao,meng,gan,ning,qing,xin,zang,jing,jin,ji1,lu,jin1,shan,yu,e,xiang,dian,gui,chuan,su,wan,zhe,fu,yu1,gan1,gang,ao,gui1,min,yue,tai,qiong from zhiwu2 where id not in (select zhiwuid from tb_classsys where del_flag = 5 and classsys_classid=26) order by id limit %s,%s'
        ...
        for foc_datas in foc_datas_rows:
            ...
            if foc_zhong is not None:
                ...
                zhong_sql = zhong_sql % (26, 'sp.', shuid, pymysql.escape_string(foc_datas[2]), '', foc_datas[3], pymysql.escape_string(foc_datas[2]), get_pinyin(foc_datas[3]),get_pinyin_prefix(foc_datas[3]),datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), foc_datas[1],datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 5, foc_datas[0])
    ...
    # for data in foc_datas:
def deal_with_zhiwu():
    books = select('select * from tb_book where remarks is not null', 'zhiwu', False)
    # print(books)
    num = 0
    for book in books:
        print(book[0], book[20])
        datas = select(
            'select * from tb_classsys where id in (select cid from tb_spvol where vol=\'%s\') and mark!=\'%s\' and del_flag=0' % (
            book[20], book[0]), 'zhiwu', False)
        if datas is not None:
            for data in datas:
                update('update tb_classsys set remarks=\'%s\' where id=%s' % (book[0], data[0]))
            print(len(datas))
            num = num + len(datas)
    print(num)
def deal_with_zhiwu_gengxin():
    rows = read_xlsx('C:/Users/dell/Desktop/未匹配到卷册数据New0321卷册不匹配-核对后.xlsx')
    i = 0
    num = 0
    for row in rows:
        i = i+ 1
        if i == 1:
            continue
        if row[len(row) - 1].value is not None:
            print(row[len(row) - 1].value,row[0].value)
            update('update tb_classsys set mark=\'%s\',remarks=\'%s\' where id=%s' % (row[len(row) - 1].value,row[len(row) - 1].value, row[0].value))
            num = num + 1
    print(num)
def deal_with_juance():
    datas = select('select * from tb_classsys where classsys_cname=\'%s\' and del_flag=0 and id<=72182' % ('紫草科'),'zhiwu',False)
    #if datas is not None:

def deal_sp(mings):
    kewords = ['sp.','subsp.','var.','subvar.','f.','subf.','cv.','prol.','monstr.','spp.','varr.','×','β','hybr.','inquil.']
    latin_full_list = []
    sp_index = -1
    zuozhe1_list = []
    zuozhe2_list = []
    for index, ming in enumerate(mings):
        if ming in kewords:
            if index < len(mings):
                sp_index = index
                if sp_index != len(mings) -1:
                    if ming == 'f.':
                        if mings[index + 1] == 'et' or mings[index + 1].istitle():
                            zuozhe1_list.append(ming)
                            #zuozhe1_list.append(mings[index + 1])
                            sp_index = -1
                    else:
                        latin_full_list.append(ming)
                        latin_full_list.append(mings[index + 1])
                else:
                    zuozhe1_list.append(ming)
        else:
            if index ==0 or index ==1:
                ...
                if sp_index != -1 :
                    if sp_index > 0 and index < sp_index:
                        zuozhe1_list.append(ming)
                    else:
                        if index > sp_index + 1 : zuozhe2_list.append(ming)
                else:
                    if sp_index < 0 : zuozhe1_list.append(ming)
    arrs = []
    if len(latin_full_list) > 0:arrs.append(' '.join(latin_full_list))
    else:arrs.append('')
    if len(zuozhe1_list) > 0:arrs.append(' '.join(zuozhe1_list))
    else:arrs.append('')
    if len(zuozhe2_list) > 0:arrs.append(' '.join(zuozhe2_list))
    else:arrs.append('')
    return arrs

def deal_with_gdzw():
    mark=197 #高等植物图鉴第一卷
    #datas = select('select classsys_cname from tb_classsys where classsys_par in (select id from tb_classsys where classsys_par in (select id from tb_classsys where classsys_par=1))','zhiwu',False)
    #print(datas)
    datas = '木麻黄科、三白草科、胡椒科、金粟兰科、杨柳科、胡桃科、桦木科、壳斗科、榆科、马尾树科、桑科、荨麻科、川苔草科、山龙眼科、铁青树科、山柚仔科、檀香科、桑寄生科、马兜铃科、大花草科、蛇菰科、蓼科、藜科、苋科、紫茉莉科、商陆科、番杏科、马齿苋科、落葵科、石竹科、睡莲科、金鱼藻科、昆栏树科、连香树科、毛茛科、木通科、小檗科、防己科、木兰科、蜡梅科、番荔枝科、肉豆蔻科、樟科、莲叶桐科'
    for index,row in enumerate(datas.split('、')):
        #print(index,row[0])
        ke_name_cn = row
        one_data = select('select * from tb_classsys where del_flag=0 and mark is null and create_date > \'2019-2-14\' and classsys_cname=\'%s\'' % ke_name_cn)
        print(index,ke_name_cn,one_data)
        # if one_data is not None:
        #     id = one_data[0]
        #     update('update tb_classsys set mark=%s where id=%s or classsys_par=%s' % (mark,id,id))
        #     shu_data = select(
        #         'select * from tb_classsys where classsys_par in (select id from tb_classsys where del_flag=0 and mark is null and create_date > \'2019-2-14\' and classsys_par=%s)' % id,'zhiwu',False)
        #     print(shu_data)


def deal_with_keshu():
    rows = select('select * from tb_classsys where del_flag=0 and mark=195 and classsys_par!=0','zhiwu',False)
    for index,row in enumerate(rows):
        print(index,row)
        if row is not None:
            id = row[0]
            classsys_par = row[3]
            data = select('select * from tb_classsys where del_flag=0 and id=%s' % classsys_par,'ke_shu_dict',True)
            if data is not None:
                d_row = select('select * from tb_classsys where del_flag=0 and mark=195 and classsys_cname=\'%s\'' % data[6],'zhiwu',True)
                if d_row is not None:
                    new_parid = d_row[0]
                    update('update tb_classsys set classsys_par=%s where id=%s' % (str(new_parid),id))


def deal_with_foc_juance():
    rows = read_xlsx('C:/Users/dell/Desktop/FOC更新.xlsx')
    for index,row in enumerate(rows):
        print(index,row[4].value)
        if index == 0:continue
        if row[4].value is not None:
            sql = "insert into tb_book (`book_name`,`publish_date`,`isbn`,`issue`,`issue_small`,`author`,`responsible_editor`,`series_name`,`volume_name`,`fascicle_no`,`men`,`gang`,`relates_ke`,`create_date`,`create_by`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
            sql = sql % (row[6].value,row[2].value,row[4].value,'',row[5].value,row[1].value,row[8].value,'中国植物志（英文版）',row[6].value.split(' ')[4],'','','','',datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),'lx',0)
            print(sql)
            insertMysql(sql)
def deal_foc_tbclasssys():
    datas = select('select * from tb_book where create_date > \'2019-04-03\'','zhiwu',False)
    for index,data in enumerate(datas):
        #print(index,data[10])
        juance_no = data[10]
        zhiwu2 = select('select count(*) from zhiwu2 where juan=\'%s\'' % (juance_no))
        tb = select('select count(*) from tb_classsys where zhiwuid is not null and id > 72182 and mark is not null and mark=\'%s\'' % (juance_no))
        #print(index,data[10],zhiwu2[0],tb[0],zhiwu2,tb)
        if zhiwu2[0] == tb[0] and zhiwu2[0] !=0 :
            print(index,data[10],zhiwu2[0],tb[0],zhiwu2,tb)
            update('update tb_classsys set mark=\'%s\',del_flag=0 where zhiwuid is not null and id > 72182 and mark is not null and mark=\'%s\'' % (data[0],juance_no) )
def check_gaodeng(path):
    wb = openpyxl.load_workbook(path)
    sheets = wb.sheetnames
    sheet = sheet = wb.get_sheet_by_name(sheets[0])
    for index,row in enumerate(sheet.rows):
        name_cn = row[4].value
        ke_data_existed = get_name_existed_create_date(name_cn, False)
        if ke_data_existed is None: print(name_cn,index,row[0].value)

    wb.close()
def zhiwutujian(path):
    datas = select('select classsys_cname from tb_classsys where create_date > \'2019-04-17\' and classsys_cname in (select classsys_cname from tb_classsys where id in (22,20,26,18,23,24,1147,1221,25,19,21) order by classsys_cname) order by classsys_cname','zhiwu',False)
    ke_list = list(ke[0] for ke in datas)
    wb = openpyxl.load_workbook(path)
    sheets = wb.sheetnames
    sheet = sheet = wb.get_sheet_by_name(sheets[0])
    rows = sheet.rows
    for index,row in enumerate(rows):
        if row[4].value is None:continue
        if index == 1:
            print(row[5].value)
            mark = row[5].value
            for ke in row[4].value.split('、'):
                ke_list.append(ke)
            for index,ke in enumerate(ke_list):
                data = select('select * from tb_classsys where create_date > \'2019-04-17\' and classsys_cname=\'%s\' and del_flag=0' % ke)
                if data is None:print(index,ke)
                else:
                    print(data[0])
                    update('update tb_classsys set mark=%s where create_date > \'2019-04-17\' and id=%s' % (mark,data[0]) )
                    update(
                        'update tb_classsys set mark=%s where create_date > \'2019-04-17\' and classsys_par=%s' % (mark, data[0]))
                    datas = select('select * from tb_classsys where create_date > \'2019-04-17\' and classsys_par=%s and del_flag=0' % data[0],'zhiwu',False)
                    if datas is not None:
                        for ds in datas:
                            print(ds[0])
                            update('update tb_classsys set mark=%s where create_date > \'2019-04-17\' and classsys_par=%s' % (str(mark), ds[0]))
        if index > 1:
            print(row[5].value)
            mark = row[5].value
            for ke in row[4].value.split('、'):
                data = select(
                    'select * from tb_classsys where create_date > \'2019-04-17\' and classsys_cname=\'%s\' and del_flag=0' % ke)
                if data is None: print(index, ke)
                else:
                    print(data[0])
                    update(
                        'update tb_classsys set mark=%s where create_date > \'2019-04-17\' and id=%s' % (mark, data[0]))
                    update(
                        'update tb_classsys set mark=%s where create_date > \'2019-04-17\' and classsys_par=%s' % (
                        mark, data[0]))
                    datas = select(
                        'select * from tb_classsys where create_date > \'2019-04-17\' and classsys_par=%s and del_flag=0' % data[0],'zhiwu',False)
                    if datas is not None:
                        for d in datas:
                            update(
                                'update tb_classsys set mark=%s where create_date > \'2019-04-17\' and classsys_par = %s' % (str(mark), d[0]))
...
def deal_with_no_mark():
    datas = select('select * from tb_classsys where id in (select distinct classsys_par from tb_classsys where del_flag=0 and mark is null and create_date > \'2019-04-17\') and classsys_par=0','zhiwu',False)
    for index,data in enumerate(datas):
        mark = data[12]
        id = data[0]
        if mark != '' or mark is not None:
            print(id,mark)
            data_shus = select('select * from tb_classsys where classsys_par = %s' % id,'zhiwu',False)
            if data_shus is not None:
                for index1,data_row in enumerate(data_shus):
                    data_species = select('select * from tb_classsys where classsys_par = %s' % data_row[0], 'zhiwu', False)
                    update('update tb_classsys set mark=%s,update_by=\'lx\' where id=%s or classsys_par=%s' % (mark, data_row[0],data_row[0]))
                    print(data_species)



def deal_with_xiuzheng(path):
    wb = openpyxl.load_workbook(path)
    sheets = wb.sheetnames
    sheet = sheet = wb.get_sheet_by_name(sheets[0])
    rows = sheet.rows
    for index, row in enumerate(rows):
        if index == 0 :continue
        #print(len(row))
        right_ke = row[7].value
        mark = row[12].value
        if right_ke is None or right_ke == '':
            #print(index, row[0].value, row[6].value, row[7].value, row[12].value)

            if mark != '' or mark is not None:
                update('update tb_classsys set mark=%s where id = %s' % (mark, row[0].value))
        else:
            #print('---',index, row[0].value, row[6].value, row[7].value, row[12].value)
            data = select(
                'select * from tb_classsys where classsys_cname = \'%s\' and create_date > \'2019-04-17\'' % right_ke)
            if data is None:
                print(right_ke)
                ke_data = get_name_existed_create_date(right_ke)
                print('科信息：', ke_data)
                if ke_data is not None:
                    sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                    sql = sql % (13, '科', 0, ke_data[4], ke_data[5], ke_data[6], ke_data[4], get_pinyin(right_ke),
                                 get_pinyin_prefix(right_ke),
                                 datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                 datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
                    ke_id = insertMysql(sql)
                    print(sql)
                    update('update tb_classsys set classsys_par=%s where classsys_par=%s' % (ke_id, row[0].value))
                else:
                    sql = "insert into tb_classsys (`classsys_classid`,`classsys_class`,`classsys_par`,`classsys_latin`,`classsys_author`,`classsys_cname`,`classsys_latin2`,`classsys_pinyin`,`classsys_py`,`classsys_addtime`,`create_date`,`del_flag`) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                    sql = sql % (13, '科', 0, '', '', right_ke, '', get_pinyin(right_ke),
                                 get_pinyin_prefix(right_ke),
                                 datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                 datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
                    ke_id = insertMysql(sql)
...
if __name__ == '__main__':
    ...
    #deal_with_zhiwu_gengxin()
    #zhiwu_book_relates()
    #read07_excel('C:/Users/dell/Desktop/中国高等植物高等彩九卷-更新.xlsx')
    # latin= 'Nabalus tatarinowii (Maxim.) Nakai subsp. macrantha (Stebbins) N. Kilian'
    # mings = latin.split()
    # arr = deal_sp(mings)
    # print(arr)
    # print(';'.join(arr[1:len(arr)]))
    # print('shu = ', mings[0])
    #read07_excel_gdzwtj('F:\Files\My Files\桌面\中国高等植物图鉴.xlsx')
    #get_text_docx()
    #deal_with_gdzwcstj()
    #deal_with_keshu()
    #deal_with_gdzw()
    #deal_with_foc_juance()
    #deal_foc_tbclasssys()

    #read07_excel_by('C:/Users/dell/Desktop/中国高等植物图鉴-更新0411.xlsx')

    #check_gaodeng('C:/Users/dell/Desktop/中国高等植物图鉴-更新0411.xlsx')

    #zhiwutujian('C:/Users/dell/Desktop/版权页整理-高等植物图鉴-new1.xlsx');

    #deal_with_xiuzheng('C:/Users/dell/Desktop/高等植物图鉴未匹配卷册的科数据.xlsx')
